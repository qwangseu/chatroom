...
#define SOCKET_TYPE_INVALID    0 //init status which means usefully.
#define SOCKET_TYPE_RESERVE    1 //get id from reaserve-id function.
#define SOCKET_TYPE_PLISTEN    2
#define SOCKET_TYPE_LISTEN     3
#define SOCKET_TYPE_CONNECTING 4
#define SOCKET_TYPE_CONNECTED  5
#define SOCKET_TYPE_HALFCLOSE  6 //will be closed after sending all data
...
struct socket {
    int fd;
    int id;            
    int type;             //SOCKET_TYPE_xxx
    unsigned int size;    //default is MIN_READ_BUFFER(the growth of the double.)
    int64_t wb_size;      //the buffer-size of the total
    uintptr_t opaque;
...
struct socket_server {
    ...
    int event_n;      //event number
    int event_index;  //the index of current event which to be deal
    struct event ev[MAX_EVENT];    // store fired-events message
    struct socket slot[MAX_SOCKET];// a lot of struct socket*/
    char buffer[MAX_INFO];         //store tmp-data
    fd_set rfds;                   //detect the pipe[0] state
};

struct request_open {
    int id;           //the index of socket_server->slot[]
    int port;
    uintptr_t opaque;
    char host[1];
};
...
send_buffer(struct socket_server *ss, struct socket *s, struct socket_message *result) {
    ...
    s->tail = NULL;
    /*send all data , and remove EPOLLOUT-event from epoll*/
...
// corresponding "O" cmd : open a socket which will connect to a server(addr:port)
// return (-1 when connecting | SOCKET_OPEN when success | SOCKET_ERROR when error) 
static int
...
// corresponding "D" cmd : send data to a socket-fd
static int
send_socket(struct socket_server *ss, struct request_send * request, struct socket_message *result) {
    ...
    if (s->head == NULL) {
        ...
        // if data is too large , if will be send later.
...
// corresponding "L" cmd
// ps: do_listen is called in socket_server_listen.
static int
listen_socket(struct socket_server *ss, struct request_listen * request, struct socket_message *result) {
    int id = request->id;
    int listen_fd = request->fd;
    struct socket *s = new_fd(ss, id, listen_fd, request->opaque, false);//notes here -> "false"
    if (s == NULL) {
        goto _failed;
    }
...
// corresponding "K" cmd : try to close socket-fd
static int
...
// message come from client-socket
...
forward_message(struct socket_server *ss, struct socket *s, struct socket_message * result) {
    ...
    if (n==0) {//means that the FIN arrived
        FREE(buffer);
        force_close(ss, s, result);
        return SOCKET_CLOSE;
    }
...
// report the socket connect success or faild
static int
